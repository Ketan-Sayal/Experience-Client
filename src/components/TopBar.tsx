      ...
      <div className="w-full px-4 py-4 sm:px-6 md:px-8 lg:px-12 xl:px-24 flex items-center justify-between shadow-lg bg-white">
        <div className="flex gap-2 sm:gap-3 items-center">
          ...
          {user._id!=null? <>
            ...
            className="hover:bg-black hover:text-yellow-600 duration-200 md:block hidden" text="Dashboard" onlyYellow={false} startIcon={<Admin/>}/>
            </>
          ):(<>
            <Button text="Login" onlyYellow={false} className="bg-black text-yellow-600 duration-200 md:block hidden" onClick={()=>navigate("/login")} endIcon={<Login/>}/>
            <Button text="Signup" onlyYellow={false} className="hover:bg-black hover:text-yellow-600 md:block hidden duration-200" onClick={()=>navigate("/signup")} endIcon={<Signup/>}/>
            </>
          )}
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <Input ref={searchRef} placeholder="Search here" className="bg-gray-300 border-2 border-gray-700 outline-none w-32 sm:w-48 md:w-64 hidden md:block"/>
          <Button text="Search" className="max-w-fit hidden md:block" onClick={()=>{
    ...
    <div className="px-6 py-5 flex flex-col gap-4">
      ...
      <div className="flex gap-2">
        <Input 
          ...
          className="bg-white border-2 border-gray-300 focus:border-yellow-600 outline-none flex-1 rounded-lg transition-all duration-200 shadow-sm"
        />
        <Button 
          text="Search" 
        ...
        {user._id!=null? (
          ...
          <>
            <Button 
              ...
              curve="rounded-lg"
              onlyYellow={false} 
              className="bg-gradient-to-r from-gray-900 to-black text-yellow-400 hover:from-black hover:to-gray-900 shadow-md hover:shadow-lg transform hover:scale-[1.02] transition-all duration-200 w-full py-3 font-semibold" 
              onClick={()=>{
                navigate("/login");
                setOpen(false);
              }} 
